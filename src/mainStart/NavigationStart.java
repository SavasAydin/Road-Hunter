...
/**
 * This class controls the flow once a navigation option is chosen from the Road
 * Hunter main menu
 * 
...
public class NavigationStart {
	...
	/**
	 * startNavigationAtoB() first option. Sets the start and end node chosen by
	 * the pedestrian and then calls reader class to start
	 */
	...
	/**
	 * startNavigationCurrentToB() second option. Sets the end node chosen by
	 * the pedestrian and then calls reader class to start. Start node will be
	 * set to pedestrians current location.
	 */
	...
	/**
	 * startTrackMovement() third option. No start and end nodes are set. This
	 * is a free movement mode so that the pedestrian can just track his/her
	 * movement.
	 */
	 ...
	 * 
	 * Within the if and else-if statements there are loops to control guidance
	 * of the pedestrian by checking when the user arrives at different nodes
	 * along the path.
	 */
	public static void calculateNaviagte() {
		...
		if (startNode == null && endNode != null) {
			...
			int xCurr = PositionCalc.calcX(Double
					.parseDouble(Reader.gpsData.longitude));
			int yCurr = PositionCalc.calcY(Double
					.parseDouble(Reader.gpsData.latitude));
			Sleep.sleep1000();
			...
			while (nextNode != sizeIndex) {
				...
				longComparsion = PositionCalc.calcX(currentLong)
						- PositionCalc.calcX(Dijkstra.path.get(nextNode)
								.getLongitude());
				longComparsion = Math.abs(longComparsion);

				latComparsion = PositionCalc.calcY(currentLat)
						- PositionCalc.calcY(Dijkstra.path.get(nextNode)
								.getLatitude());
				...
				if ((longComparsion + latComparsion) < 15) {
					if (nextTurn < Directions.directionList.size()) {
						MapWindowJFrame
								.setDirectionIcon(Directions.directionList
										.get(nextTurn));
						nextTurn++;
			...
			while (nextNode == sizeIndex) {
				...
				if ((longComparsion + latComparsion) < 15) {
					System.out
							.println("You've arrived at your final destination");
	...
	/**
	 * setNodes() After a completed navigation this method is called to prepare
	 * the arraylists for new paths
	 */
