...
public class MapView {
	...
	/**
	 * A method to paint the nodes of the route calculated by the Dijkstra
	 * class. Receives calculated route as an ArrayList
	 */
	...
	/**
	 * A method to paint the path of the route calculated by the Dijkstra class
	 * onto the map. Receives calculated route as an ArrayList
	 */
	public void paintRoute(ArrayList<Vertex> vertexList) {
		...
		for (int i = 0; i < vertexList.size(); i++) {
			...
			if (i > 0) {
				...
				int xOld = PositionCalc.calcX(vertexList.get(i - 1)
						.getLongitude());

				int yOld = PositionCalc.calcY(vertexList.get(i - 1)
						.getLatitude());
	...
	/**
	 * Method to set position for the start and end makers on to the map, they
	 * will appear centered on the start and end nodes
	 */
	public void EndFlag(ArrayList<Vertex> path) {
		...
		g2d.drawImage(image,
				...
				PositionCalc.calcY(path.get(0).getLatitude()) - 10, null);
	...
	/**
	 * Method displaying the total distance of path from current location to end
	 * node in the map view status bar.
	 */
	...
	public void viewPosition1(double longitude, double latitude) {
		...
		/**
		 * Calls a brake for a 1ms just so that the paint method won't screw up
		 */

		Sleep.sleep1();

		/**
		 * Paints the tracing dot, the minus values are there to get the center
		 * of the point on the coordinate
		 */
	...
	/**
	 * viewPosition() method takes pedestrian's current location coordinate and
	 * displays it on the map and stays as a trace
	 * 
	...
	public void viewPosition(double xIn, double yIn) {
		...
		Image image = picfile.getImage();
		g2d.drawImage(image, x - 10, // Just to center the image which is 20x20
										// px in size
